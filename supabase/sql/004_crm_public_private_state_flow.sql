-- 004_crm_public_private_state_flow.sql
-- Pulido de flujo:
-- 1) CIERRE_SOLICITADO queda como estado interno del CRM.
-- 2) SEPARADO ya no se crea desde cambio manual de estado.
--    Para separar un lote debe usarse crm_crear_separacion().
...
begin
  ...
  if p_estado_nuevo = 'SEPARADO' then
    raise exception 'Para separar un lote crea una separacion con cliente.';
  end if;
