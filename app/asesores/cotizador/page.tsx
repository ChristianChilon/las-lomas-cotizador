...
export default function Home() {
  ...
  useEffect(() => {
    const actualizarZoomInicial = () => {
      const esMovil = esPantallaTactil(window.innerWidth);

      setZoomMovil(esMovil);
      setEscalaInicialPlano(
        calcularEscalaEncuadre(window.innerWidth, window.innerHeight)
      );
    };
  ...
  useEffect(() => {
    ...
    const cargarLotes = async () => {
      try {
        if (!supabase) {
          setErrorLotes("Supabase no esta configurado.");
          return;
        }
        ...
        console.error(error);
        setErrorLotes(
          "No se pudieron cargar los precios privados. Actualiza la pagina o revisa tu sesion."
        );
      }
    };
...
Quisiera más información.`;
  ...
  if (verificandoSesion || !autorizado) {
    return (
      <main className="cotizador-private-loading">
        <Image
          src="/icon.png"
          alt="Las Lomas"
          width={72}
          ...
          priority
        />
        <strong>Verificando acceso al cotizador...</strong>
      </main>
    );
  }
